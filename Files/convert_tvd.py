...
def old(netlist, iternum): # Converts the original circuit to verilog
...
def new(netlist, iternum): # Creates an encrypted circuit in verilog
    keys = []
    srb = []
    srb.append(" isb")
    ...
    inputs, outputs, gates = p.parse(netlist)
    selected_gates = p.select_gates(netlist, "highest")
    ga = [g[1] for g in gates]
    ...
    with open(netlist[:-6] + ("_%d" % iternum) + "_hybrid.v", "w") as file:
        ...
        for g in gates:
            ...
            if g[1] in selected_gates:
                ...
                i4 = "~(%s ^ %s)" % (a, b)
                i5 = "%s ^ %s" % (a, b)
                file.write("\tassign %s = (%s & ~%s & ~%s & ~%s) | (%s & ~%s & ~%s & %s) | (%s & ~%s & %s & ~%s) | (%s & ~%s & %s & %s) | (%s & %s & ~%s & ~%s) | (%s & %s & ~%s & %s);\n" % (g[1], i0, keyinputs[0], keyinputs[1], keyinputs[2], i1, keyinputs[0], keyinputs[1], keyinputs[2], i2, keyinputs[0], keyinputs[1], keyinputs[2], i3, keyinputs[0], keyinputs[1], keyinputs[2], i4, keyinputs[0], keyinputs[1], keyinputs[2], i5, keyinputs[0], keyinputs[1], keyinputs[2]))
...
def tb(netlist, iternum): # Creates a testbench used in icarus Verilog
    inputs, outputs, gates = p.parse(netlist)
    inputs = list(inputs)
